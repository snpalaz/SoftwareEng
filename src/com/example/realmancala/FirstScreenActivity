...
public class FirstScreenActivity extends Activity {
	...
	private void doFirstRun() {
		...
		if (settings.getBoolean("isFirstRun", true)) {
			Toast.makeText(FirstScreenActivity.this, "First run",
			...
			Toast.makeText(FirstScreenActivity.this, "İlk açılış değil",
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	public void AddFirstRun() {
		...
		try {
			...
			didItWork = false;
			isWork = "Veri tabanı oluşturulamadı!";

		} finally {
			if (didItWork) {
				isWork = "İşlem başarılı";
			}
		}
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		doFirstRun();//Uygulamayı ilk çalıştırma 
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK)// donanımsal olarak geri butonuna
		// basılırsa
		{
